   ...
   for (i = 0; i < 3; i++) {
      ...
      for (j = 0; j < 10; j++) {
         ...
         if (oldMonth != newDate.month) {
            ...
            content += "<br><span class='text-danger text-monthname'> "+(gregMonths[newDate.month-1]+"</span><span class='text-danger'> "+newDate.day)+"</span>";
         } else {
...
function makeGregorianCalendarContent(year, month) {
   ...
   for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (oldMonth != tmpDate.month) {
         ...
         calendar += "<span>" + i + "</span><br><span class='text-danger text-monthname'> "+(months[tmpDate.month-1]+"</span><span class='text-danger'> "+tmpDate.day)+"</span></td>";
      } else {
